...
class LrNativeConnectionActive {
	constructor(backend, proxy) {
		if (!("connectNative" in browser.runtime)) {
			throw new Error("No permission for native apps");
		}
		this.port = browser.runtime.connectNative(backend);
		this.proxy = proxy;
...
// FIXME clear this._abortPromise on disconnect
	...
	async _getAbortPromise() {
		if (this._abortPromise === undefined) {
			try {
				...
				// FIXME use this._lock field instead of global object
				if (
